...
#[test]
fn dexter_stake_unstake_test() -> Result<(), RuntimeError> {
    // Arrange
    ...
    // Turn the nft claim receipt into a regular bucket
    let nft_claim_receipt_regular_bucket = Bucket::from(nft_claim_receipt);
    ...
    println!(
        "NFT Claim Receipt: {:?}",
        nft_claim_receipt_regular_bucket.amount(&mut env)?
    );
